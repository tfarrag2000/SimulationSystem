...
def genetic_algorithm(simulation, 
    ...
    for iteration in range(num_generations):
        ...
        active_nodes_history.append(int(np.sum(best_particle[:, 2])))
...
def simulated_annealing(simulation, initial_temp=100, cooling_rate=0.95, 
    ...
    for iteration in range(iterations):
        ...
        coverage, avg_overlap = simulation.compute_coverage(best_solution)
        coverage_history.append(coverage * 100)
        overlap_history.append(avg_overlap)
        active_nodes_history.append(int(np.sum(best_solution)))
...
def genetic_algorithm_with_sa(
    ...
    """Genetic Algorithm for drone optimization with Simulated Annealing refinement"""
    start_time = time.time()
    AreaWidth, AreaHeight = simulation.width, simulation.height
    SensingRange = simulation.sensing_radius
    NumNodes = len(simulation.drones)
    ...
    for iteration in range(num_generations):
        try:
            fitness_scores = parallel_fitness_evaluation(population, fitness, num_processes=cpu_count())
        except:
            fitness_scores = [fitness(p) for p in population]
        sorted_indices = np.argsort(fitness_scores)[::-1]
        population = [population[i] for i in sorted_indices]
        ...
        overlap_history.append(calculate_overlap(best_particle))
        active_nodes_history.append(int(np.sum(best_particle[:, 2])))
        new_population = population[:int(elitism_fraction * population_size)]
        while len(new_population) < population_size:
            parent1, parent2 = random.sample(population[:min(30, len(population))], 2)
            child1, child2 = crossover(parent1, parent2)
            new_population.append(mutate(child1))
    ...
    num_sa_iterations = sa_iters
    temp = sa_temp
    cooling_rate = sa_cooling
    best_solution = best_particle.copy()
    best_fitness = fitness(best_solution)
    for iteration in range(num_sa_iterations):
    ...
    result = AlgorithmResult(
        ...
        active_nodes=int(np.sum(best_solution[:, 2])),
