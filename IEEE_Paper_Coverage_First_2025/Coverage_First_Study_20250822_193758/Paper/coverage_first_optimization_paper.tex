...
\usepackage{algps\begin{figure}[t]
\centering
\includegraphics[width=0.48\textwidth]{../Figures/PNG/dashboard_screenshot.png}
\caption{Interactive Dashboard showing PSO Smart optimization results with 16/25 active drones achieving 100\% coverage and 36\% energy savings. The interface demonstrates real-time visualization of active drones (green), sleeping drones (gray), and coverage areas (light green overlay).}
\label{fig:dashboard}
\end{figure}de}
\usepackage{cite}
\usepackage{booktabs}
...
\textbf{Configuration Panel}: Provides intuitive parameter configuration including test case selection, algorithm switching with visual indicators, environment setup (grid dimensions, drone count, coverage radius), and parallel processing controls with automatic CPU core detection.

\textbf{Advanced Settings Panel}: Enables sophisticated optimization control through convergence parameters (maximum iterations, threshold settings), intelligent early stopping mechanisms, energy efficiency mode with configurable targets, and dynamic active/sleep drone management for energy conservation.

\textbf{Visualization Panel}: Delivers real-time graphical representation featuring interactive grid displays with color-coded drone states, coverage area overlays, live performance metrics, and time-series convergence monitoring.
...
\subsection{Advanced Visualization Features}

The dashboard implements several sophisticated capabilities for comprehensive optimization analysis:

\textbf{Real-time Coverage Mapping}: Dynamic overlay visualization with transparency effects indicating coverage intensity and gap detection.

\textbf{Energy Efficiency Tracking}: Integrated metrics showing optimization progress, energy savings calculation, and active/sleep drone ratios with visual indicators.

\textbf{Algorithm Performance Monitoring}: Live convergence tracking, parameter sensitivity analysis, and comparative algorithm evaluation with statistical reporting.

\subsection{Practical Implementation Results}

Figure \ref{fig:dashboard} demonstrates the dashboard's capability in displaying optimization results from the Smart PSO algorithm. The visualization shows optimal deployment achieving 100\% coverage using only 16 of 25 available drones (64\% utilization), resulting in 36\% energy savings while maintaining complete area monitoring. The green circles represent active drones providing coverage, gray circles indicate sleeping drones conserving energy, and the light green overlay visualizes the comprehensive coverage area.
...
\subsection{Research and Deployment Applications}

The visualization framework supports various research methodologies and operational requirements:

\begin{itemize}
\item \textbf{Algorithm Comparison}: Real-time evaluation of different optimization approaches with immediate visual feedback
\item \textbf{Parameter Optimization}: Interactive adjustment of algorithm parameters with instant performance visualization
\item \textbf{Deployment Validation}: Visual confirmation of coverage adequacy and energy efficiency before field deployment
\item \textbf{Performance Monitoring}: Continuous tracking of system efficiency and optimization quality metrics
\end{itemize}
...
\item \textbf{Interactive Validation}: Dashboard interface enables real-time optimization verification and deployment confidence
\end{itemize}
